...
const TripExpense = ({ tripExpenses, expenseList, totalExpense }) => {
  ...
  function renderNetPayments() {
    return Object.keys(memberNetPayments).map(id => {
      return (
        <MemberPaymentListItem key={`net-payment-${id}`}>
          <div>{`${memberNetPayments[id][0]} ${
  ...
  function renderPayments() {
    return Object.keys(memberPayments).map(id => {
      return (
        <MemberPaymentListItem key={`total-payment-paid-${id}`}>
          <div>{`${memberPayments[id][0]} paid`}</div>
          <div>{convertNumberToCurrency(memberPayments[id][1])}</div>
        </MemberPaymentListItem>
      );
    });
  }
...
const MemberPaymentList = styled.ul`
  padding: 0;
  margin: 0;
  list-style: none;
  font-size: 16px;
...
const MemberPaymentListItem = styled.li`
  padding: 0 10px;
  margin: 5px 0;
  font-size: 14px;
  display: flex;
